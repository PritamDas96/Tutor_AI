...
#   2) read_url    (HTML/PDF)    -> {'title','url','text'}
#   3) rag_retrieve(FAISS + BGE) -> [{'title','url','chunk','score'}]
# Agent: Structured Chat (multi-arg tools) with Hugging Face LLM
# RAG: top_k=7 with rerank; curated public sources
# Observability: LangSmith tracing + thumbs feedback

import os, io, json, hashlib, requests
...
SCENARIOS: Dict[str, Dict[str, str]] = {
    ...
    "Automating Everyday Tasks with GenAI": {
        "overview": "Drafting, idea generation, structuring notes, time-saving workflows.",
...
def cached_search(query: str, max_results: int = 5) -> List[Dict[str, str]]:
    ...
    try:
        ...
        out = [{"error": f"Web search failed: {e}"}]
    return out

@lc_tool("web_search", args_schema=WebSearchInput)
def web_search_tool(query: str, max_results: int = 5) -> list[dict]:
    """Search the web (DuckDuckGo) and return a list of results with title, url, and snippet."""
...
def hf_direct_reply(model_id: str, user_text: str, system_text: str = "") -> str:
    """Fallback HF chat completion ensuring a response even if the agent fails."""
    try:
...
def get_agent(model_id: str, use_tools: bool, max_iterations: int):
    ...
    # Include {tools} and {tool_names}; scratchpad as assistant text to avoid type mismatch
    ...
    executor = AgentExecutor(
        agent=agent,
        tools=tools,
        verbose=True,                    # helpful for debugging + LangSmith traces
    ...
    with tracing_context(project_name=LS_PROJECT, metadata={
        ...
        with trace("agent_turn", run_type="chain", inputs={"question": user_q}):
            with st.chat_message("assistant"):
                ...
                # Agent trace (tool calls) with unique keys for text_area
            ...
            st.session_state.turn_logs.append({
                ...
                "question": user_q,
                "answer": reply,
                "contexts": [],  # optionally fill from rag_retrieve obs
            })
