...
@st.cache_resource(show_spinner=True)
def load_reranker():
    # lighter & faster than bge-m3, still decent
...
def build_rag_index(doc_links: List[Dict[str, Any]]):
    ...
    for doc in doc_links:
        ...
        if not raw or len(raw.split()) < 100:
            # keep going; don't let one bad source kill the build
            continue
...
def retrieve(query: str, index, side: Dict[str, Any], top_k: int = TOP_K) -> Tuple[List[Dict[str, Any]], float]:
    ...
    # Rerank (or proxy if unavailable)
    try:
...
def web_search(query: str, max_results: int = 8) -> List[Dict[str, str]]:
    ...
    try:
        ddg = get_ddg()
        for r in ddg.text(keywords=query, max_results=max_results, region="us-en", safesearch="moderate"):
    ...
    # simple broadening fallback that does NOT assume ddg exists
    if not results and len(query.split()) > 2:
        simple_query = re.sub(r"\b(2024|2025|in gen ?ai)\b", "", query, flags=re.I).strip()
        simple_query = " ".join([w for w in simple_query.split() if len(w) > 3])
        if simple_query:
            try:
                ddg = ddg or get_ddg()
                for r in ddg.text(keywords=simple_query, max_results=max_results, region="us-en", safesearch="moderate"):
                    title = r.get("title") or ""
                    url = r.get("href") or r.get("url") or ""
                    snippet = r.get("body") or ""
                    if title and url:
...
def _force_last_json(text: str) -> Dict[str, Any]:
    """Final-attempt coercion to salvage JSON commands."""
    t = text.strip()
...
def hf_chat(model: str, messages: List[Dict[str,str]], max_new_tokens=512, temperature=0.0, top_p=0.9) -> str:
    ...
    # Add stop hints to reduce extra chatter beyond JSON
    resp = client.chat_completion(
        messages=messages,
        max_tokens=max_new_tokens,
        temperature=temperature,
        top_p=top_p,
        stop=["```", "\n\nObservation:", "\n\nUser:"],  # ignored by some models, harmless
    )
...
"""
        ...
        with trace("planner_step", run_type="chain", inputs={"step": step, "evidence_count": len(ev.rag_chunks) + len(ev.pages)}):
            ...
            # If still nothing useful, run a safe default tool ONCE instead of counting a strike
            if not parsed or not any(k in parsed for k in ("tool","stop","thought")):
